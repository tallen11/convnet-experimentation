...
class ModelStandardV2(Model):
    def __init__(self, activation_func):
        super(ModelStandardV2, self).__init__("standard_v2", activation_func)
        with tf.device("/gpu:0"):
            ...
            W_conv2 = self.__weights([5,5,128,256])
            b_conv2 = self.__biases([256])
            ...
            W_conv3 = self.__weights([5,5,256,256])
            b_conv3 = self.__biases([256])
            ...
            W_fc1 = self.__weights([4*4*256, 2048])
            b_fc1 = self.__biases([2048])
            h_pool3_flat = tf.reshape(h_pool3_dropout, [-1, 4*4*256])
            h_fc1 = self.act_fn(tf.nn.xw_plus_b(h_pool3_flat, W_fc1, b_fc1))
            h_fc1_dropout = tf.nn.dropout(h_fc1, self.dropout)

            W_fc2 = self.__weights([2048, 1024])
            b_fc2 = self.__biases([1024])
            h_fc2 = self.act_fn(tf.nn.xw_plus_b(h_fc1_dropout, W_fc2, b_fc2))
            h_fc2_dropout = tf.nn.dropout(h_fc2, self.dropout)

            W_fc3 = self.__weights([1024, 10])
